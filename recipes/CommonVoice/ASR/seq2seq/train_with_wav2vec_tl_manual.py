...
if __name__ == "__main__":
    ...
    asr_pt = torch.load(hparams["pretrained_dir"])
    for key in list(asr_pt.keys()):
        asr_pt[key.replace('1.', ''). replace('2.', ''). replace('3.', ''). replace('4.', '')] = asr_pt.pop(key)
    
    # hparams["emb"] = sb.nnet.embedding.Embedding (
    #     num_embeddings=hparams["output_neurons"],
    #     embedding_dim=hparams["emb_size"]
    # )
    # emb_state_dict = {key:value for key, value in asr_pt.items() if key in list(hparams["emb"].state_dict().keys())}
    # hparams["emb"].load_state_dict(emb_state_dict)
    # hparams["emb"].train()
    
    hparams["dec"] = sb.nnet.RNN.AttentionalRNNDecoder(
        enc_dim=hparams["dnn_neurons"],
        input_size=hparams["emb_size"],
        rnn_type="gru",
        attn_type="location",
        hidden_size=1024,
        attn_dim=1024,
    ...
    dec_state_dict = {key:value for key, value in asr_pt.items() if key in list(hparams["dec"].state_dict().keys())}
    # print(hparams["dec"].state_dict().keys())
    # print(asr_pt.keys())
    hparams["dec"].load_state_dict(dec_state_dict)
    hparams["dec"].train()
    ...
    hparams["model"] = torch.nn.ModuleList(
        [hparams["enc"], hparams["emb"], hparams["dec"], hparams["ctc_lin"], hparams["seq_lin"]]
    )
    
    hparams["beam_searcher"] = sb.decoders.S2SRNNBeamSearcher(
        ...
        eos_threshold=hparams["eos_threshold"],
        using_max_attn_shift=hparams["using_max_attn_shift"],
        max_attn_shift=hparams["max_attn_shift"],
        temperature=hparams["temperature"],
        ctc_score_mode=hparams["ctc_score_mode"],
    ...
    hparams["checkpointer"]=sb.utils.checkpoints.Checkpointer(
        ...
        recoverables={
            "wav2vec2": hparams["wav2vec2"],
            "model": hparams["model"],
            "scheduler_model": hparams["lr_annealing_model"],
            "scheduler_wav2vec": hparams["lr_annealing_wav2vec"],
